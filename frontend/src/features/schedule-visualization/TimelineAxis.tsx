...
export function TimelineAxis({ domain, width, height }: Props) {
  ...
  return (
    <g>
      <line x1={0} y1={height - 1} x2={width} y2={height - 1} stroke="currentColor" strokeOpacity="0.18" />
      {ticks.map((minute) => {
        const x = getScaleX(minute, domain, width);

        return (
          <g key={minute} transform={`translate(${x}, 0)`}>
            <line y1={height - 8} y2={height} stroke="currentColor" strokeOpacity="0.36" />
            <text
              y={12}
              textAnchor={minute === domain.start ? "start" : minute === domain.end ? "end" : "middle"}
              className="fill-slate-700 text-[9px] sm:text-[10px]"
            >
              {formatHour(minute)}
            </text>
          </g>
